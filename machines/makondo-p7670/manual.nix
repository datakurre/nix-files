...
in
{
  ...
  #
  # Wrap OBS with the PRIME render-offload env vars so the NVIDIA GPU handles
  # encoding (NVENC), while the desktop remains on the iGPU. The variables are
  # the same ones set by the nvidia-offload command (generated by
  # hardware.nvidia.prime.offload.enableOffloadCmd = true).
