    ...
    programs.nushell.extraConfig = ''
            ...
            def "ask-ai" [
              ...
              let spinner_pid = if not $verbose {
                ...
                let cmd = ($"parent=($parent_pid)\n" + '
      (
        trap "printf \"\\r\\033[2K\" >&2; exit 0" INT TERM HUP
        count=0
        while [ $count -lt 3000 ] && kill -0 $parent 2>/dev/null; do
          for c in "/" "-" "\\" "|"; do
            printf "\r\033[2K%s Asking AI..." "$c" >&2
            sleep 0.1
            count=$((count + 1))
          done
        done
      ...
      ')
              ...
              let response = try {
                ...
                if not $verbose {
                  if $spinner_pid > 0 { try { ^kill -s TERM $spinner_pid } }
